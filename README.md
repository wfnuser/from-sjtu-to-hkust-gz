...
- 默认网页现为 Day 1–16 江西执行线，产物位于 `web/data/inland-execution-*`，路线与每日住宿配置分别位于 `config/inland-execution-route.json` 和 `config/inland-itinerary.json`。
- 公开行程约 1556.9 km；Day 3–16 剩余约 1340.5 km，骑行日均约 95.7 km。Day 3 实际规划保持为海创园亚朵经桐庐捷安特到建德新安江麗枫，约 134.7 km。
- Day 4 为建德麗枫直达常山东方广场酒店，约 113.8 km。Day 7–15 重新均衡：Day 7 = 维也纳南城（91 km）、Day 8 = 头陂（112 km，唯一长日）、Day 9 = 仙下（106 km）、Day 10 = 韩坊（99 km ✓）、Day 11 = 定南（107 km）、Day 12 = 漳溪（101 km）、Day 13–15 均 ≤ 100 km；Day 16 = 番禺亚运城维也纳 → 香港科技大学（广州）短抵达日，约 29 km，8 月 29 日抵达。
- 16 天上限仅适用于 execution profile（coastal / inland 仍为 15 天硬约束）；Day 0 是上海 prelude，不计入骑行日。
- Day 7–16 每天从前一晚酒店直达当天酒店，不设置固定中途吃饭、充电或休整点。Day 7 已住（维也纳酒店 南城，2026-08-20 实际入住）；小镇酒店的自助洗衣仍须在订房前电话确认，页面不会将其误标为已确认。
- 左栏展示 Day 1–16 日卡；点击日卡缩放并高亮当天道路。地图使用高德中文底图，浏览器内将发布的 WGS-84 路线转换到 GCJ-02 对齐；手机竖屏采用上方地图、下方独立滚动日程面板，桌面使用左侧日程、右侧地图。
...
- 默认 Day 1–16 江西执行线：<http://127.0.0.1:8765/>
