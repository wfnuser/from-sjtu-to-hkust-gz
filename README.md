...
- 默认网页现为 Day 1–15 江西执行线，产物位于 `web/data/inland-execution-*`，路线与每日住宿配置分别位于 `config/inland-execution-route.json` 和 `config/inland-itinerary.json`。
- 公开行程约 1521.9 km；Day 3–15 剩余约 1340.1 km，骑行日均约 103.1 km。Day 3 实际规划保持为海创园亚朵经桐庐捷安特到建德新安江麗枫，约 134.7 km。
- Day 4 为建德麗枫直达常山东方广场酒店，约 113.8 km。Day 6–15 重新均衡为约 94–107 km，并在 Day 15 抵达香港科技大学（广州）。
- Day 6–15 每天从前一晚酒店直达当天酒店，不设置固定中途吃饭、充电或休整点。鹰潭终点为枫丹白露酒店（现雲锦君澜）；七晚小镇酒店的自助洗衣仍须在订房前电话确认，页面不会将其误标为已确认。
- 左栏只展示 Day 1–15 日卡；点击日卡缩放并高亮当天道路。地图使用高德中文底图，浏览器内将发布的 WGS-84 路线转换到 GCJ-02 对齐；手机竖屏采用上方地图、下方独立滚动日程面板，桌面使用左侧日程、右侧地图。
...
- 默认 Day 1–15 江西执行线：<http://127.0.0.1:8765/>
